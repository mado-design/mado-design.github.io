...
function createMiniNav(item) {
  ...
  const variant = (miniNavVariants.get(item) || 0) % 4;

  if (!link) {
    link = document.createElement("a");
    link.className = "mini-nav-link";
    link.href = item.getAttribute("href") || "#home";
...
function checkMiniNavBlocks() {
  ...
  navItems.forEach((item) => {
    ...
    const hitFromBelow = previousKeyboardTop >= rect.bottom - 8 && logoRect.top <= rect.bottom;
