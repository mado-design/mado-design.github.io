...
function buildSliceBoard() {
  ...
  function getCoverageBonus(box) {
    ...
    const maxDistance = Math.hypot(boardCenterX, boardCenterY) || 1;
  ...
  function getOverflowPenalty(box) {
    ...
    const overflowRight = Math.max(0, box.right - window.innerWidth);
    const overflowTop = Math.max(0, -box.top);
    const overflowBottom = Math.max(0, box.bottom - window.innerHeight);

    return (overflowLeft + overflowRight + overflowTop + overflowBottom) * 100;
  }

  function keepPlacementInBounds(left, top, width, aspectRatio, rotate) {
    let nextLeft = left;
    let nextTop = top;

    for (let pass = 0; pass < 3; pass += 1) {
      const box = getStickerBox(nextLeft, nextTop, width, aspectRatio, rotate);
      let shiftX = 0;
      let shiftY = 0;

      if (box.width > window.innerWidth) {
        shiftX = window.innerWidth / 2 - (box.left + box.width / 2);
      } else if (box.left < 0) {
        shiftX = -box.left;
      } else if (box.right > window.innerWidth) {
        shiftX = window.innerWidth - box.right;
      }

      if (box.height > window.innerHeight) {
        shiftY = window.innerHeight / 2 - (box.top + box.height / 2);
      } else if (box.top < 0) {
        shiftY = -box.top;
      } else if (box.bottom > window.innerHeight) {
        shiftY = window.innerHeight - box.bottom;
      }

      if (Math.abs(shiftX) < 0.5 && Math.abs(shiftY) < 0.5) {
        return { left: nextLeft, top: nextTop, box };
      }

      nextLeft += (shiftX / window.innerWidth) * 100;
      nextTop += (shiftY / window.innerHeight) * 100;
    }

    return {
      left: Math.max(0, Math.min(nextLeft, 100)),
      top: Math.max(0, Math.min(nextTop, 100)),
      box: getStickerBox(nextLeft, nextTop, width, aspectRatio, rotate),
    };
  }

  function limitInitialSliceWidth(width, aspectRatio) {
    const maxWidth = maxSliceWidthRatio * 100;

    if (aspectRatio >= 1) {
      return Math.min(width, maxWidth);
    }

    const widthPx = (width / 100) * window.innerWidth;
    const heightPx = widthPx / aspectRatio;
    const maxHeightPx = window.innerHeight * maxSliceHeightRatio;

    if (heightPx <= maxHeightPx) {
      return width;
    }

    return ((maxHeightPx * aspectRatio) / window.innerWidth) * 100;
  }
  ...
  selected.forEach((file, index) => {
    ...
    const randomWidth = isVideo ? 7.5 + random() * 7.5 : 15 + random() * 15;
    const width = override?.width ?? limitInitialSliceWidth(randomWidth, aspectRatio);
    ...
    if (override?.left !== undefined && override?.top !== undefined) {
      ...
      const placementAttempts = isTransparentImage ? 120 : 88;
      const overlapWeight = isTransparentImage ? 5.4 : 3.2;
      const coverageWeight = isTransparentImage ? 0.2 : 0.14;

      for (let attempt = 0; attempt < placementAttempts; attempt += 1) {
        const rotateDirection = index % 2 === 0 ? -1 : 1;
        const rotate = rotateDirection * (8 + random() * 30);
        const rawLeft = random() * Math.max(8, 92 - width);
        const rawTop = random() * 76 + 8;
        const placement = keepPlacementInBounds(rawLeft, rawTop, width, aspectRatio, rotate);
        const score = getOverlapScore(placement.box) * overlapWeight - getCoverageBonus(placement.box) * coverageWeight + random() * 0.16;
...
function limitInitialRenderedSliceSize(sticker) {
  ...
  const maxHeight = window.innerHeight * maxSliceHeightRatio;
